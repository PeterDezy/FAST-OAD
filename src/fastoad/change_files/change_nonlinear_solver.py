...
def broyden_change():
    ...
    def save(b):
        ...
        try:
            ...
            doc["model"]["nonlinear_solver"] += (
                ",debug_print="
                + str(debug_print.value)
                + ",stall_limit="
                + str(stall_limit.value)
                + ",stall_tol="
                + str(stall_tol.value)
                + ",alpha="
                + str(alpha.value)
            ...
            doc["model"]["nonlinear_solver"] += (
                ",compute_jacobian="
                + str(compute_jacobian.value)
                + ",converge_limit="
                + str(converge_limit.value)
                + ",cs_reconverge="
                + str(cs_reconverge.value)
            )
            doc["model"]["nonlinear_solver"] += (
                ",diverge_limit="
                + str(diverge_limit.value)
                + ",max_converge_failures="
                + str(max_converge_failures.value)
                + ",max_jacobians="
                + str(max_jacobians.value)
            )
            doc["model"]["nonlinear_solver"] += (
                ",state_vars="
                + str(state_vars.value)
                + ",update_broyden="
                + str(update_broyden.value)
                + ",reraise_child_analysiserror="
                + str(reraise_child_analysiserror.value)
                + ")"
            )
    ...
    state_vars = widgets.Text(value="[]", description="State Vars :", disabled=False)
    ...
    button = widgets.Button(description="Save", tooltip="Save", icon="save")

    left_box = widgets.VBox(
        [maxiter, rtol, err_on_non_converge, stall_limit, alpha, converge_limit]
    )
...
def newton_change():
    ...
    left_box = widgets.VBox(
        [maxiter, rtol, err_on_non_converge, stall_limit, solve_subsystems, cs_reconverge]
    )
    right_box = widgets.VBox(
        [atol, iprint, debug_print, stall_tol, vb, reraise_child_analysiserror]
    )
    vbox = widgets.HBox([left_box, right_box])
